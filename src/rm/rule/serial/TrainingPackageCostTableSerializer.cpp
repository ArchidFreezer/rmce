...
std::string TrainingPackageCostTableSerializer::serializeObject(const rm::rule::table::TrainingPackageCostTable& ref) const {
	std::stringstream ss;
	...
	std::map<const PersistentMatcher*, TableRow<int>> rows = ref.modified();
	for (const auto& [matcher, row] : rows) {
		sorted_rows[matcher->gameData()->id()] = row;
		if (row.colCount() > num_cols) {
			num_cols = row.colCount();
		}
	}
...
const rm::rule::table::TrainingPackageCostTable& TrainingPackageCostTableSerializer::deserializeObject(std::istream& is) const {
	using namespace rm::rule::table;
