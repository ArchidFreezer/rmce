...
void RaceSerializer::parse(pt::ptree& ptree) {
	std::cout << "Loading Race data ..." << std::endl;

	// Get the races to parse and loop through them
	const pt::ptree& tree = ptree.get_child(rootNode());
	for (const auto& v : tree) {
		std::string name = v.second.get<std::string>("name");
		std::string id = v.second.get("id", manager().generateId(ruleDatatype(), name));

		// Get a RaceData object from the object manager
		RaceData& ref = manager().get<RaceData>(id);
		ref.setName(name);
		...
		ref.setStartingLanguageAbilities(converter.parseLanguageAbilityMapTree(v.second.get_child_optional("starting-languages")));

		// Get the adolescent languages
		ref.setAdolescentLanguageAbilities(converter.parseLanguageAbilityMapTree(v.second.get_child_optional("adolescent-languages")));

		// Get Stat bonuses
		ref.setStatBonuses(converter.parseEnumPairTree<StatType::Type, int>(v.second.get_child_optional("stat-bonuses")));

		// Get everyman skills
		ref.setEverymanSkills(converter.parseSkillSetTree(v.second.get_child_optional("everyman-skills")));

		// Get restricted skills
		ref.setRestrictedSkills(converter.parseSkillSetTree(v.second.get_child_optional("restricted-skills")));

		// Everyman skill categories
		ref.setEverymanSkillCategories(converter.parseGameDataSetTree<SkillCategoryData>(v.second.get_child_optional("everyman-categories")));

		// Restricted skill categories
		ref.setRestrictedSkillCategories(converter.parseGameDataSetTree<SkillCategoryData>(v.second.get_child_optional("restricted-categories")));
		...
		ref.setCategoryEverymanSkillChoices(converter.parseGameDataChoiceSetTree<SkillCategoryData>(v.second.get_child_optional("skill-category-choices-everyman")));
...
void RaceSerializer::populateDatum(std::string key, pt::ptree& datum) {
	RaceData& game_data = manager().get<RaceData>(key);
	...
	// Starting languages
	{
		pt::ptree tree{converter.getLanguageAbilityMapTree(game_data.startingLanguageAbilities())};
		if (tree.size())
			datum.push_back(std::make_pair("starting-languages", tree));
	}

	// Adolescent languages
	{
		pt::ptree tree{converter.getLanguageAbilityMapTree(game_data.adolescentLanguageAbilities())};
		if (tree.size())
			datum.push_back(std::make_pair("adolescent-languages", tree));
	}

	// Stat bonus
	{
		pt::ptree tree{converter.getEnumPairTree<StatType::Type, int>(game_data.statBonuses())};
		if (tree.size())
			datum.push_back(std::make_pair("stat-bonuses", tree));
	}

	// Everyman skills
	{
		pt::ptree tree{converter.getSkillSetTree(game_data.everymanSkills())};
		if (tree.size())
			datum.push_back(std::make_pair("everyman-skills", tree));
	}

	// Restricted skills
	{
		pt::ptree tree{converter.getSkillSetTree(game_data.restrictedSkills())};
		if (tree.size())
			datum.push_back(std::make_pair("restricted-skills", tree));
	}

	// Everyman categories
	{
		pt::ptree tree{converter.getGameDataSetTree<SkillCategoryData>(game_data.everymanSkillCategories())};
		if (tree.size())
			datum.push_back(std::make_pair("everyman-categories", tree));
	}

	// Restricted categories
	{
		pt::ptree tree{converter.getGameDataSetTree<SkillCategoryData>(game_data.restrictedSkillCategories())};
		if (tree.size())
			datum.push_back(std::make_pair("restricted-categories", tree));
	}
	...
	{
		pt::ptree tree{converter.getGameDataChoiceSetTree<SkillCategoryData>(game_data.categoryEverymanSkillChoices())};
		if (tree.size())
			datum.push_back(std::make_pair("skill-category-choices-everyman", tree));
	}
}

std::string RaceSerializer::rootNode() const {
	return "races";
}
