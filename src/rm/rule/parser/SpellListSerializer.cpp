#include <SpellListSerializer.h>

namespace rm::rule::parser {

void SpellListSerializer::parse(pt::ptree& ptree) {
	std::cout << "Loading SpellList data ..." << std::endl;

	// Get the spelllists to parse and loop through them
	const pt::ptree& tree = ptree.get_child(rootNode());
	for (const auto& v : tree) {
		std::string name = v.second.get<std::string>("name");
		std::string id = v.second.get("id", manager().generateId(ruleDatatype(), name));

		// Get a SpellListData object from the object manager
		SpellListData& ref = manager().get<SpellListData>(id);
		ref.setName(name);
		std::string book_id = v.second.get<std::string>("book");
		ref.setBook(manager().get<BookData>(book_id));

		ref.setIsEvil(v.second.get<bool>("evil"));
		ref.setIsSummoning(v.second.get<bool>("summoning"));

		// Get the spell list type
		std::optional<SpellListType::Type> type = SpellListType::fromString(v.second.get<std::string>("type"));
		if (type)
			ref.setType(type.value());

		// Get the spell power realms
		for (const auto& realm_tree : v.second.get_child("realms")) {
			std::optional<RealmType::Type> realm = RealmType::fromString(realm_tree.second.get_value<std::string>());
			if (realm)
				ref.addRealm(realm.value());
		}

		std::cout << "\tSpellList name: " << ref.name() << std::endl;
	}
	std::cout << " done" << std::endl;
}

void SpellListSerializer::populateDatum(std::string key, pt::ptree& datum) {
	SpellListData& game_data = manager().get<SpellListData>(key);
	datum.put("id", game_data.id());
	datum.put("name", game_data.name());
	datum.put("book", game_data.book().value()->id());
	datum.put("type", SpellListType::toString(game_data.type()));
	datum.put("evil", game_data.isEvil());
	datum.put("summoning", game_data.isSummoning());

	// Get the container tree for the realms
	pt::ptree realms_tree{};
	for (RealmType::Type realm : game_data.realms()) {
		// Get the realm container
		pt::ptree realm_tree{};
		realm_tree.put("", RealmType::toString(realm));
		realms_tree.push_back(std::make_pair("", realm_tree));
	}
	datum.push_back(std::make_pair("realms", realms_tree));
}

std::string SpellListSerializer::rootNode() const {
	return "spell-lists";
}

} // namespace rm::rule::parser