...
void SkillSerializer::parse(pt::ptree& ptree) {
	std::cout << "Loading Skill data ..." << std::endl;

	// Get the skills to parse and loop through them
	const pt::ptree& tree = ptree.get_child(rootNode());
	for (const auto& v : tree) {
		std::string name = v.second.get<std::string>("name");
		std::string id = v.second.get("id", manager().generateId(ruleDatatype(), name));

		// Get a SkillData object from the object manager
		SkillData& ref = manager().get<SkillData>(id);
		ref.setName(name);
		...
		ref.setBook(manager().get<BookData>(book));
		...
		ref.setCategory(manager().get<SkillCategoryData>(category));
		...
		ref.setSubcategories(converter.parseSetTree<std::string>(v.second.get_child_optional("subcategories")));
...
void SkillSerializer::populateDatum(std::string key, pt::ptree& datum) {
	SkillData& game_data = manager().get<SkillData>(key);
	...
	// Subcategories
	{
		pt::ptree tree{converter.getSetTree<std::string>(game_data.subcategories())};
		if (!tree.empty())
			datum.push_back(std::make_pair("subcategories", tree));
	}
...
std::string SkillSerializer::rootNode() const {
	return "skills";
}
