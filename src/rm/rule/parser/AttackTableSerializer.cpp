...
void AttackTableSerializer::parse(pt::ptree& ptree) {
	...
	for (const auto& ptable : tree) {
		...
		rm::rule::table::AttackTable& table = manager().get<rm::rule::table::AttackTable>(id);
...
void AttackTableSerializer::populateDatum(std::string key, pt::ptree& datum) {
	rm::rule::table::AttackTable& game_data = manager().get<rm::rule::table::AttackTable>(key);
...
std::string AttackTableSerializer::rootNode() const {
