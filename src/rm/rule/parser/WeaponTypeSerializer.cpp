...
void WeaponTypeSerializer::parse(pt::ptree& ptree) {
	std::cout << "Loading WeaponType data ..." << std::endl;
	...
	for (const auto& v : tree) {
		...
		WeaponTypeData& ref = manager().get<WeaponTypeData>(id);
		ref.setName(name);
		...
		ref.setSkill(manager().get<SkillData>(skill_id));

		// attack table
		std::string attack_table_id = v.second.get<std::string>("attack-table");
		ref.setAttackTable(manager().get<rm::rule::table::AttackTable>(attack_table_id));
...
void WeaponTypeSerializer::populateDatum(std::string key, pt::ptree& datum) {
	WeaponTypeData& game_data = manager().get<WeaponTypeData>(key);
	...
	if (!pranges.empty())
		datum.push_back(std::make_pair("ranges", pranges));
}

std::string WeaponTypeSerializer::rootNode() const {
	return "weapon-types";
}
