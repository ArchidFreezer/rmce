...
void PoisonTypeSerializer::parse(pt::ptree& ptree) {
	...
	for (const auto& v : tree) {
		...
		// Get a PoisonTypeData object from the object manager
		PoisonTypeData& ref = manager().get<PoisonTypeData>(id);
...
void PoisonTypeSerializer::populateDatum(std::string key, pt::ptree& datum) {
	PoisonTypeData& game_data = manager().get<PoisonTypeData>(key);
	...
	// Effect onset times
	{
		...
		if (tree.size())
			datum.push_back(std::make_pair("severity-effect-onsets", tree));
	}
...
std::string PoisonTypeSerializer::rootNode() const {
	return "poison-types";
}
