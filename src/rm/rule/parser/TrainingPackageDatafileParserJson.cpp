...
namespace rm::rule::parser {
	...
	void TrainingPackageDatafileParserJson::parse() {
		...
		for (const auto& v : tree) {
			...
			if (language_choices_tree) { ref.setLanguageChoices(parseGameDataChoicePairTree<LanguageData, int>(language_choices_tree)); }
	...
	void TrainingPackageDatafileParserJson::populateDatum(std::string& id, pt::ptree& datum) {
		...
		// Language choices
		{
			pt::ptree tree{ getGameDataChoicePairTree<LanguageData, int>(game_data.languageChoices()) };
			if (tree.size()) datum.push_back(std::make_pair("language-choices", tree));
		}
	}
	...
	std::set<CategoryMultiSkillRankChoice> TrainingPackageDatafileParserJson::parseCategoryMultiSkillRankChoices(boost::optional<const pt::ptree&> category_multi_skill_rank_choices) {
		...
		for (const auto& choice : category_multi_skill_rank_choices.get()) {
			...
			choice_struct.category = &factory().get<SkillCategoryData>(category_id);
	...
	const pt::ptree TrainingPackageDatafileParserJson::getCategoryMultiSkillRankChoicesTree(TrainingPackageData& game_data) {
		pt::ptree choice_tree{};
	...
	std::set<SpellListCategoryChoices> TrainingPackageDatafileParserJson::parseSpellListCategoryChoicesTree(boost::optional<const pt::ptree&> spell_list_category_choices) {
		std::set<SpellListCategoryChoices> choices{};
		if (!spell_list_category_choices) return choices;
		...
		for (const auto& choice : spell_list_category_choices.get()) {
			...
			if (spell_lists_tree) {
				for (const auto& spell_list : spell_lists_tree.get()) {
					std::string spell_list_id = spell_list.second.get_value<std::string>();
					choice_struct.spell_list_categories.emplace(&factory().get<SkillCategoryData>(spell_list_id));
				}
			}
			choices.emplace(choice_struct);
		}
	...
	const pt::ptree TrainingPackageDatafileParserJson::getSpellListCategoryChoicesTree(TrainingPackageData& game_data) {
		pt::ptree choice_tree{};
