...
std::set<const SubcategoriedSkillData*> GameRuleBoostConverter::parseSkillSetTree(boost::optional<const pt::ptree&> tree) {
	...
	if (tree) {
		for (const auto& items : tree.get()) {
			...
			if (subcategory) {
				datum.insert(&manager_.subcategoriedSkillData(id, subcategory.get()));
			} else {
				datum.insert(&manager_.subcategoriedSkillData(id));
			}
		}
	}
	return datum;
...
const pt::ptree GameRuleBoostConverter::getSkillSetTree(std::set<const SubcategoriedSkillData*> set) {
	pt::ptree tree{};
...
std::map<std::string, const rm::game::character::LanguageAbility> GameRuleBoostConverter::parseLanguageAbilityMapTree(boost::optional<const pt::ptree&> tree) {
	...
	if (tree) {
		for (const auto& items : tree.get()) {
			...
			rm::game::character::LanguageAbility ability(manager_.get<LanguageData>(language_name));
			boost::optional<int> somantic = items.second.get_optional<int>("somantic");
			if (somantic) {
				ability.updateSomanticRanks(somantic.get());
			}
			boost::optional<int> spoken = items.second.get_optional<int>("spoken");
			if (spoken) {
				ability.updateSpokenRanks(spoken.get());
			}
			boost::optional<int> written = items.second.get_optional<int>("written");
			if (written) {
				ability.updateWrittenRanks(written.get());
			}
...
const pt::ptree GameRuleBoostConverter::getLanguageAbilityMapTree(std::map<std::string, const rm::game::character::LanguageAbility> map) {
	pt::ptree tree{};
	for (const auto& pair : map) {
		pt::ptree value_tree{};
		value_tree.put("language", pair.second.languageId());
		if (pair.second.somantic())
			value_tree.put("somantic", pair.second.somantic());
		if (pair.second.spoken())
			value_tree.put("spoken", pair.second.spoken());
		if (pair.second.written())
			value_tree.put("written", pair.second.written());
		tree.push_back(std::make_pair("", value_tree));
	}
	return tree;
}

std::set<GameRuleDataChoice<SubcategoriedSkillData>> GameRuleBoostConverter::parseSkillChoiceSetTree(boost::optional<const pt::ptree&> tree) {
	...
	if (tree) {
		for (const auto& choice_tree : tree.get()) {
			...
			for (const auto& options_tree : choice_tree.second.get_child("options")) {
				...
				if (subcategory) {
					choice_data.addOption(manager_.subcategoriedSkillData(skill_id, subcategory.get()));
				} else {
					choice_data.addOption(manager_.subcategoriedSkillData(skill_id));
				}
			}
...
const pt::ptree GameRuleBoostConverter::getSkillChoiceSetTree(std::set<GameRuleDataChoice<SubcategoriedSkillData>> set) {
	pt::ptree tree{};
	for (const GameRuleDataChoice<SubcategoriedSkillData>& item : set) {
		...
		for (const auto& pair : sorted_options) {
			...
			if (pair.second->subcategory())
				option_tree.put("subcategory", pair.second->subcategory().value());
