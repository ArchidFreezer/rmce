...
namespace rm::rule::enums {
	...
	const std::optional<CreatureBonusXpType::Type> CreatureBonusXpType::fromString(std::string_view sv) {
		...
		if (sv.empty() || sv == "None" || sv == "-") { return kNone; } else if (sv.size() == 1 && sv[0] == 'A') { return kA; } else if (sv.size() == 1 && sv[0] == 'B') { return kB; } else if (sv.size() == 1 && sv[0] == 'C') { return kC; } else if (sv.size() == 1 && sv[0] == 'D') { return kD; } else if (sv.size() == 1 && sv[0] == 'E') { return kE; } else if (sv.size() == 1 && sv[0] == 'F') { return kF; } else if (sv.size() == 1 && sv[0] == 'G') { return kG; } else if (sv.size() == 1 && sv[0] == 'H') { return kH; } else if (sv.size() == 1 && sv[0] == 'I') { return kI; } else if (sv.size() == 1 && sv[0] == 'J') { return kJ; } else if (sv.size() == 1 && sv[0] == 'K') { return kK; } else if (sv.size() == 1 && sv[0] == 'L') { return kL; } else throw std::invalid_argument("Invalid string value for CreatureBonusXpType::Type: " + std::string(sv));
