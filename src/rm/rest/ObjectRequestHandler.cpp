...
void ObjectRequestHandler::handleRequest(const http::request<http::string_body>& request, http::response<http::string_body>& response) {
	...
	// Get the type prefix from the path (e.g. "character" from "/rmce/objects/character")
	std::string type_prefix = std::string(path.extractNextSegment("/rmce/objects/"));
	// Get the ID from the path if it exists (e.g. "CHARACTER_1" from "/rmce/objects/character/CHARACTER_1")
	std::string id = type_prefix.empty() ? "" : std::string(path.extractNextSegment("/rmce/objects/" + type_prefix + "/"));

	/***************************
	 * Handle GET requests
	 */
	// Get the count of objects of a specific types (e.g. /rmce/objects/count?types=character,race)
	...
	// Get a list of all known type prefixes for objects (e.g. /rmce/objects/prefixes)
		...
		requestTypes(response);
	}

	// Get a list of all objects of a specific type (e.g. /rmce/objects/character)
	else if (request.method() == http::verb::get && !type_prefix.empty() && id.empty() && path.params().empty()) {
		requestListObjects(response, type_prefix);
	}

	// Get a list of all object IDs of a specific type (e.g. /rmce/objects/character?ids)
	...
	// Get the count of objects of a specific type (e.g. /rmce/objects/character?count)
	...
	// Get a specific object by ID (e.g. /rmce/objects/character/CHARACTER_1)
	 ...
	 */
	// Create a new object of a specific type (e.g. POST /rmce/objects/character with JSON body containing the object data)
	 ...
	 * Handle PUT requests
	 */
	// Update an existing object by ID (e.g. PUT /rmce/objects/character/CHARACTER_1 with JSON body containing the updated object data)
	 ...
	 */
	// Delete an existing object by ID (e.g. DELETE /rmce/objects/character/CHARACTER_1)
...
void ObjectRequestHandler::requestTypes(http::response<http::string_body>& response) {
	response.result(http::status::ok);
	response.set(http::field::content_type, "application/json");

	try {
		std::set<std::string> prefixes = serial_manager_.objectManager().getAllObjectLabels();
...
void ObjectRequestHandler::requestObjectById(http::response<http::string_body>& response, std::string_view type, std::string_view id) {
	response.set(http::field::content_type, "application/json");
	try {
		std::string obj_json_str = serial_manager_.serializeAnyGameObject(id, type);
