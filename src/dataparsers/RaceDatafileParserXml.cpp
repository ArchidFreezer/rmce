...
void RaceDatafileParserXml::parse(bool id_only) {
	...
	for (const auto& v : tree) {
		...
		for (const auto& realm_progression : v.second.get_child("realm-progressions")) {
			...
			case (RealmType::kArcane): ref.setArcaneProgression(factory().get<SkillProgressionTypeData>(progression)); break;
			case (RealmType::kArms): ref.setArmsProgression(factory().get<SkillProgressionTypeData>(progression)); break;
			case (RealmType::kChanneling): ref.setChannelingProgression(factory().get<SkillProgressionTypeData>(progression)); break;
			case (RealmType::kEssence): ref.setEssenceProgression(factory().get<SkillProgressionTypeData>(progression)); break;
			case (RealmType::kMentalism): ref.setMentalismProgression(factory().get<SkillProgressionTypeData>(progression)); break;
			}
		}
		...
		if (boost::optional<const pt::ptree&> everyman_skills = v.second.get_child_optional("skills-everyman")) {
			for (const auto& everyman_skill : everyman_skills.get()) {
				...
				if (!attr.empty()) {
					ref.addEverymanSkill(factory().subcategoriedSkillData(skill_name, attr));
				} else {
		...
		if (boost::optional<const pt::ptree&> restricted_skills = v.second.get_child_optional("skills-restricted")) {
			for (const auto& restricted_skill : restricted_skills.get()) {
				...
				if (!attr.empty()) {
					ref.addRestrictedSkill(factory().subcategoriedSkillData(skill_name, attr));
				} else {
		...
		if (boost::optional<const pt::ptree&> skill_bonuses = v.second.get_child_optional("skills-bonus")) {
			for (const auto& skill_bonus : skill_bonuses.get()) {
				...
				if (!attr.empty()) {
					ref.setSkillBonus(factory().subcategoriedSkillData(skill_name, attr), bonus);
				} else {
