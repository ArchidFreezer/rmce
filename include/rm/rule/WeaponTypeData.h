 ...
 *
 * The data includes the dimentions of a typical weapon, the attack table it uses and other data such as criticals and ranges if applicable.
 */
...
public:
	...
	inline static std::string prefix_{"WEAPONTYPE"}; /**< Prefix for the ID of the data objects */
	 ...
	 */
	void setAttackTable(const rm::rule::table::AttackTable& table) {
		table_ = &table;
	}
	 ...
	 */
	const rm::rule::table::AttackTable& attackTable() const {
		return *table_;
	}
	 ...
	 */
	void setMaxStrength(int max_strength) {
	 ...
	 */
	void setWoodenHaft(bool wooden_haft) {
		wooden_haft_ = wooden_haft;
	}
	 ...
	 */
	void addCriticalModifier(rule::enums::CriticalType::Type type, int modifier) {
		criticals_.insert(std::make_pair(type, modifier));
	}
	 ...
	 * @return int modifier
	 */
	int criticalModifier(rule::enums::CriticalType::Type critical) const {
		return criticals_.at(critical);
	}
	 ...
	 */
	void addRange(const archid::NumberRange<int>& range, int modifier) {
		ranges_.insert(std::make_pair(&range, modifier));
	}
	...
	int rangeModifier(int range) const {
		for (auto& matcher : std::views::keys(ranges_)) {
			if (matcher->matches(range))
				return ranges_.at(matcher);
		}
