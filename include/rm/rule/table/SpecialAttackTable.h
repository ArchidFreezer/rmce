 ...
 *
 * There are several attack tables in the game, mostly animal attacks, that take a dice roll is used for the rows and creature ArmourType::Type is used for the columns. The difference between these and the normal AttackTable is that the
 * these tables also have limit on the highest modified roll that sizes of attacks may use. The AttackSizeType::Type is generally based on the creature size, but is not always an exact match.
 *
 * Has the ability to handle rows that may only be used on unmodified rolls and these will be checked before any check on the normal rows that are used to match attack rolls with modifiers applied.
 *
 * Defines the minimum and maximum roll value that the table has rows for. If a roll given that is outside these values the min and max values will be used instead. This permits open ended roll results be used against the table and have a
 * row returned without being concerned about the extents of the table rows.
 *
 * Defines the type of game data object and the creation of an instance is used to set the unique element of the id. The implication of this is that any class that derives from this cannot set its own subtype.
 */
...
public:
	...
	inline static std::string prefix_{"SPECIALATTACKTABLE"}; /**< Prefix for the ID of the data objects */
	...
	std::string_view prefix() const override {
		return SpecialAttackTable::prefix_;
	}

	/**
	 * @brief Gets the value of a cell in the table
	 *
	 * Constrains the value of the \a row_index parameter to be within the bounds set on the table so that it will never be larger than the maximum bound and never smaller than the minimum bound.
	 *
	 ...
	 */
	const int limit(AttackSizeType::Type type) const {
		return limits_.at(type);
	}
