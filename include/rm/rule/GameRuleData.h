...
public:
	...
	/**
	 * @brief Generate the unique id of the game data object in a standard format
	 *
	 * The unique id is generated by combining the prefix from the derived class and the id with an underscore and converting it to uppercase. This ensures that all unique ids are in a standard format and are unique across all types of game
	 * data objects.
	 * 
	 * @return Unique identifier of the game data object as a string view
	 */
	std::string_view generateId() {
		id_ = archid::ucase(std::string(prefix()) + "_" + std::string(id_));
		return id_;
	}
