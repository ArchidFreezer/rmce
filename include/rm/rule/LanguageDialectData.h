 ...
 *
 * When reading a piece of text that includes dialect it is assumed that the reader can understand the text, but it will take longer as they need to work some owrds out from context.
 *
 * For verbal communication where understanding needs to be quicker, unless the speaker deliberately slows down their speaking the listeners skill ranks should be considered as 3/4 (round up).
 *
 * Typically the data id for a GameRuleData object would be generated from a type prefix combined with a data value, but for dialects we use the base language name to allow it to be retrievd more easily.
 *
 * @see #LanguageData
 */
...
public:
	 ...
	 */
	LanguageDialectData(std::string_view name) : GameRuleData(name), name_{name} {
	}

	inline static std::string prefix_{"LANGUAGEDIALECT"}; /**< Prefix for the ID of the data objects */
	...
	std::string_view prefix() const override {
		return LanguageDialectData::prefix_;
	}

	/**
	 * @brief Base language of the dialects
	 *
	 * It should be noted that in many cases that there is no such thing as a base language and all languages are considered dialects, e.g. English could be considered a base language with many counties/regions having their own dialect, but
	 * there is no definitive english standard.
	 *
	 ...
	 */
	const std::set<const LanguageData*>& dialects() const {
		return dialects_;
	}
