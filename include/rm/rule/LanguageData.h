 ...
 *
 * Languages can take multiple forms, written, spoken and somantic (body). For simplicity languages such as sign-language for the deaf are considered a true somantic language.
 * Players can develop in a language as with other skills using development points.
 *
 * Some languages contain dialects where users of the same language from different areas or professions may add additional words or grammar. This can hinder communication, though typically does not prevent it. For some languages there is no
 * true base, or the base is no longer in use so all languages may be considered dialects.
 *
 * Languages are also grouped in categories that represent their development from a common root language that is often a dead language that has fallen out of use.
 *
...
public:
	...
	inline static std::string prefix_{"LANGUAGE"}; /**< Prefix for the ID of the data objects */
	 ...
	 */
	void setBaseLanguage(std::string base_language) {
