 ...
 *
 * The first is the overall culture type that defines may of the general details such as cultural beliefs and fears, preferred weapons and armour plus the basic skills that are learned during adolescence by all members of the culture.
 *
 * The second is the sub culture that the character belongs to which defines the hobby skills and languages, plus any preferred professions or typical training packages, etc. These sub cultures are there to add additional flavour and
 * details. An example would be the Plains culture where a sub-cultures would define the difference between a nomadic culture and a settled culture.
 *
 * @see CultureTypeData
 */
...
public:
	...
	inline static std::string prefix_{"CULTURE"}; /**< Prefix for the ID of the data objects */
	 ...
	 */
	void setCultureType(const CultureTypeData& culture_type) {
		culture_type_ = &culture_type;
	}
	 ...
	 */
	const CultureTypeData& cultureType() const {
		return *culture_type_;
	}
	 ...
	 */
	void addHobbySkill(const SubcategoriedSkillData& skill) {
		hobby_skills_.emplace(&skill);
	}
	 ...
	 */
	void setHobbySkills(std::set<const SubcategoriedSkillData*> hobby_skills) {
		hobby_skills_ = std::move(hobby_skills);
	}
	 ...
	 */
	const std::set<const SubcategoriedSkillData*>& hobbySkills() const {
		return hobby_skills_;
	}
	 ...
	 */
	void addHobbySkillCategory(const SkillCategoryData& skill_category) {
		hobby_skill_categories_.emplace(&skill_category);
	}
	 ...
	 */
	void setHobbySkillCategories(std::set<const SkillCategoryData*> hobby_skill_categories) {
		hobby_skill_categories_ = std::move(hobby_skill_categories);
	}
	 ...
	 */
	const std::set<const SkillCategoryData*>& hobbySkillCategories() const {
		return hobby_skill_categories_;
	}
	 ...
	 */
	void addPreferredProfession(const ProfessionData& profession) {
		preferred_professions_.emplace(&profession);
	}
	 ...
	 */
	void setPreferredProfessions(std::set<const ProfessionData*> preferred_professions) {
		preferred_professions_ = std::move(preferred_professions);
	}
	 ...
	 */
	const std::set<const ProfessionData*>& preferredProfessions() const {
		return preferred_professions_;
	}
	 ...
	 * @param profession ProfessionData to add
	 */
	void addRestrictedProfession(const ProfessionData& profession) {
		restricted_professions_.emplace(&profession);
	}
	 ...
	 */
	void setRestrictedProfessions(std::set<const ProfessionData*> restricted_professions) {
		restricted_professions_ = std::move(restricted_professions);
	}
	 ...
	 * @return std::set<ProfessionData> professions
	 */
	const std::set<const ProfessionData*>& restrictedProfessions() const {
		return restricted_professions_;
	}
	 ...
	 */
	void addTrainingPackageModifier(const TrainingPackageData& training_package, float modifier) {
		training_package_modifiers_.emplace(&training_package, modifier);
	}
	 ...
	 */
	void setTrainingPackageModifiers(std::map<const TrainingPackageData*, float> training_package_modifiers) {
		training_package_modifiers_ = std::move(training_package_modifiers);
	}
	 ...
	 */
	std::map<const TrainingPackageData*, float> trainingPackageModifiers() const {
		return training_package_modifiers_;
	}
	...
	float trainingPackageModifier(const TrainingPackageData& training_package) const {
		for (const auto& key : training_package_modifiers_) {
			if (training_package.id() == key.first->id())
				return key.second;
		}
