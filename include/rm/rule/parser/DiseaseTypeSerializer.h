...
class DiseaseTypeSerializer : public PersistentObjectJsonSerializer {
public:
	/**
	 * @brief Type alias for the data class handled by this serializer
	 */
	using DataClass = DiseaseTypeData;

	/**
	 * @brief Constructor
	 * @param object_manager Reference to an object manager to handle the data objects
	 */
	DiseaseTypeSerializer(rm::PersistentObjectManager& object_manager) : PersistentObjectJsonSerializer(object_manager, "DiseaseType") {
	}
...
private:
	 ...
	 *
	 * Parse a boost::ptree containing the disease type rule data, convert to DiseaseTypeData objects and store in a data cache
	 */
	void parse(pt::ptree& ptree) override;

	/**
	 * @brief Populate the given boost tree with the data from a disease type
	 * @param key Id of the disease type to populate from
