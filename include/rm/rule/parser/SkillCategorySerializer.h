...
class SkillCategorySerializer : public PersistentObjectJsonSerializer {
public:
	/**
	 * @brief Type alias for the data class handled by this serializer
	 */
	using DataClass = SkillCategoryData;

	/**
	 * @brief Constructor
	 * @param object_manager Reference to an object manager to handle the data objects
	 */
	SkillCategorySerializer(rm::PersistentObjectManager& object_manager) : PersistentObjectJsonSerializer(object_manager, "SkillCategory") {
	}
...
private:
	 ...
	 *
	 * Parse a boost::ptree containing the skill category rule data, convert to SkillCategoryData objects and store in a data cache
	 */
	void parse(pt::ptree& ptree) override;

	/**
	 * @brief Populate the given boost tree with the data from a skill category
	 * @param key Id of the skill category to populate from
