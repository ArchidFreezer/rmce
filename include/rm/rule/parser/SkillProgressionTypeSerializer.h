 ...
 * @brief Process SkillProgressionTypeData objects for json files
 *
...
class SkillProgressionTypeSerializer : public PersistentObjectJsonSerializer {
public:
	/**
	 * @brief Type alias for the data class handled by this serializer
	 */
	using DataClass = SkillProgressionTypeData;

	/**
	 * @brief Constructor
	 * @param object_manager Reference to an object manager to handle the data objects
	 */
	SkillProgressionTypeSerializer(rm::PersistentObjectManager& object_manager) : PersistentObjectJsonSerializer(object_manager, "SkillProgressionType") {
	}
...
private:
	 ...
	 *
	 * Parse a boost::ptree containing the skill progression type rule data, convert to SkillProgressionTypeData objects and store in a data cache
	 */
	void parse(pt::ptree& ptree) override;

	/**
	 * @brief Populate the given boost tree with the data from a skill progression type
	 * @param key Id of the skill progression type to populate from
