...
public:
	...
	PersistentObjectSerializer(rm::PersistentObjectManager& object_manager, std::string_view datatype)
	    : rule_datatype_{datatype}, object_manager_{object_manager} {
	}

	/**
	 * @brief Default destructor
	 *
	 * This is made virtual to define the class as polymorphic as a standard practice.
	 */
	 ...
	 */
	const std::string& ruleDatatype() const {
		return rule_datatype_;
	}

	/**
	 * @brief Read persistent data, convert to objects and store in the persistent data cache
	 */
	virtual void read() = 0;

	/**
	 * @brief Write persistent data from the cache
	 */
	virtual void save() = 0;
	 ...
	 */
	rm::PersistentObjectManager& manager() {
