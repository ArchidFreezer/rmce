...
class PoisonTypeSerializer : public PersistentObjectJsonSerializer {
public:
	/**
	 * @brief Type alias for the data class handled by this serializer
	 */
	using DataClass = PoisonTypeData;

	/**
	 * @brief Constructor
	 * @param object_manager Reference to an object manager to handle the data objects
	 */
	PoisonTypeSerializer(rm::PersistentObjectManager& object_manager) : PersistentObjectJsonSerializer(object_manager, "PoisonType") {
	}
...
private:
	 ...
	 *
	 * Parse a boost::ptree containing the poison type rule data, convert to PoisonTypeData objects and store in a data cache
	 */
	void parse(pt::ptree& ptree) override;

	/**
	 * @brief Populate the given boost tree with the data from a poison type
	 * @param key Id of the poison type to populate from
