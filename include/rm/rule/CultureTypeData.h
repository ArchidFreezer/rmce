 ...
 *
 * These cultural influences manifest themselves as religious practices or instinctive biases for or against other races or cultures and what special skills they are likely to pick up during their formative years, represented in gameplay by
 * adolescent and hobby skill ranks.
 *
 * Within each culture type there are specifc sub cultures, such as lower class or militant versions that further refine these. The sub culture tends to define hobby preferences or languages and these are defined in the CultureData class
...
public:
	...
	inline static std::string prefix_{"CULTURETYPE"}; /**< Prefix for the ID of the data objects */
	...
	std::string_view prefix() const override {
		return CultureTypeData::prefix_;
	}
	 ...
	 */
	void setCharacterConcept(std::string_view concepts) {
		character_concepts_ = concepts;
	}
	 ...
	 */
	const std::string& characterConcept() const {
		return character_concepts_;
	}
	 ...
	 */
	void setClothing(std::string_view clothing) {
		clothing_ = clothing;
	}
	 ...
	 */
	const std::string& clothing() const {
		return clothing_;
	}
	 ...
	 */
	void setAspirations(std::string_view aspirations) {
		aspirations_ = aspirations;
	}
	 ...
	 */
	const std::string& aspirations() const {
		return aspirations_;
	}
	 ...
	 */
	void setFears(std::string_view fears) {
		fears_ = fears;
	}
	 ...
	 */
	void setMarriagePatterns(std::string_view marriage_patterns) {
		marriage_patterns_ = marriage_patterns;
	}
	 ...
	 */
	const std::string& marriagePatterns() const {
		return marriage_patterns_;
	}
	 ...
	 */
	void setPrejudices(std::string_view prejudices) {
		prejudices_ = prejudices;
	}
	 ...
	 */
	const std::string& prejudices() const {
		return prejudices_;
	}
	 ...
	 */
	void setReligiousBeliefs(std::string_view religious_beliefs) {
		religion_ = religious_beliefs;
	}
	 ...
	 */
	const std::string& religiousBeliefs() const {
		return religion_;
	}
	 ...
	 */
	void setHobbySkillRanks(int hobby_skill_ranks) {
		hobby_skill_ranks_ = hobby_skill_ranks;
	}
	 ...
	 */
	int hobbySkillRanks() const {
		return hobby_skill_ranks_;
	}
	 ...
	 */
	void setSpellListRanks(int spell_list_ranks) {
		spell_list_ranks_ = spell_list_ranks;
	}
	 ...
	 */
	int spellListRanks() const {
		return spell_list_ranks_;
	}
	 ...
	 */
	void addPreferredWeapon(WeaponTypeData& weapon) {
		preferred_weapons_.emplace(&weapon);
	}
	 ...
	 */
	const std::set<const WeaponTypeData*> preferredWeapons() const {
		return preferred_weapons_;
	}
	 ...
	 */
	void setPreferredWeapons(std::set<const WeaponTypeData*> weapons) {
		preferred_weapons_ = std::move(weapons);
	}
	 ...
	 */
	void addSkillCategoryRank(const SkillCategoryData& category, int ranks) {
		skill_category_ranks_.emplace(&category, ranks);
	}
	 ...
	 */
	void setSkillCategoryRanks(std::map<const SkillCategoryData*, int> map) {
		skill_category_ranks_ = std::move(map);
	}
	...
	int skillCategoryRank(const SkillCategoryData& category) {
		for (auto& cat : std::views::keys(skill_category_ranks_)) {
			if (cat->id() == category.id())
				return skill_category_ranks_.at(cat);
		}
		return 0;
	}
	 ...
	 */
	void addSkillCategorySkillRank(const SkillCategoryData& category, int ranks) {
		skill_category_skill_ranks_.emplace(&category, ranks);
	}
	 ...
	 */
	void setSkillCategorySkillRanks(std::map<const SkillCategoryData*, int> map) {
		skill_category_skill_ranks_ = std::move(map);
	}
	 ...
	 */
	const std::map<const SkillCategoryData*, int>& skillCategorySkillRanks() const {
		return skill_category_skill_ranks_;
	}
	...
	int skillCategorySkillRank(const SkillCategoryData& category) {
		for (auto& cat : std::views::keys(skill_category_skill_ranks_)) {
			if (cat->id() == category.id())
				return skill_category_skill_ranks_.at(cat);
		}
		return 0;
	}
	 ...
	 */
	void addRequiredFeature(EnvironmentType::Feature feature) {
		required_features_.emplace(feature);
	}
	 ...
	 */
	void setRequiredFeatures(std::set<EnvironmentType::Feature> features) {
	 ...
	 */
	const std::set<EnvironmentType::Feature> requiredFeatures() const {
	 ...
	 */
	bool isRequiredFeature(EnvironmentType::Feature feature) const {
		return (required_features_.find(feature) != required_features_.end());
	}
	 ...
	 */
	void addRequiredTerrain(EnvironmentType::Terrain terrain) {
		required_terrains_.emplace(terrain);
	}
	 ...
	 */
	void setRequiredTerrains(std::set<EnvironmentType::Terrain> terrains) {
		required_terrains_ = std::move(terrains);
	}
	 ...
	 */
	const std::set<EnvironmentType::Terrain> requiredTerrains() const {
		return required_terrains_;
	}
	 ...
	 */
	bool isRequiredTerrain(EnvironmentType::Terrain terrain) const {
		return (required_terrains_.find(terrain) != required_terrains_.end());
	}
	 ...
	 */
	void addRequiredVegetation(EnvironmentType::Vegetation vegetation) {
		required_vegetations_.emplace(vegetation);
	}
	 ...
	 */
	void setRequiredVegetations(std::set<EnvironmentType::Vegetation> vegetations) {
		required_vegetations_ = std::move(vegetations);
	}
	 ...
	 */
	const std::set<EnvironmentType::Vegetation> requiredVegetations() const {
		return required_vegetations_;
	}
	 ...
	 */
	bool isRequiredVegetation(EnvironmentType::Vegetation vegetation) const {
		return (required_vegetations_.find(vegetation) != required_vegetations_.end());
	}
	 ...
	 */
	void addRequiredWaterSource(EnvironmentType::Water water) {
		required_water_sources_.emplace(water);
	}
	 ...
	 */
	void setRequiredWaterSources(std::set<EnvironmentType::Water> water_sources) {
		required_water_sources_ = std::move(water_sources);
	}
	 ...
	 */
	const std::set<EnvironmentType::Water> requiredWaterSources() const {
		return required_water_sources_;
	}
	 ...
	 */
	bool isRequiredWaterSource(EnvironmentType::Water water) const {
		return (required_water_sources_.find(water) != required_water_sources_.end());
	}
