 ...
 * @brief A training package is a group of benefits (and sometimes disadvantages) that can be developed using development points
 *
 * The skills and abilities gained through a package represent long periods of exposure or training (sometimes years). If these packages are developed after apprenticeship development, the character should be required to spend time in
 * training (the time will vary from package to package).
 *
 * There are two different types of training packages: vocational and lifestyle. Most of the time, a character may have only one lifestyle training package (GM discretion). However, he can have as many vocational training packages as he can
 * afford to purchase.
 *
 * <em>Lifestyle</em> - These training packages represent groups of skills and abilities that would be gained by living a particular lifestyle. Lifestyle packages should be hard to acquire, and take long periods of time when they are
 * allowed.
 *
 * <em>Vocational</em> - These training packages represent specific occupations in which a character may become proficient. He will gain groups of skills and abilities that would necessarily be developed if the character were in the
 * occupation.
 */
...
public:
	...
	inline static std::string prefix_{"TRAININGPACKAGE"}; /**< Prefix for the ID of the data objects */
	...
	std::string_view prefix() const override {
		return TrainingPackageData::prefix_;
	}
	 ...
	 */
	void setFlavourText(std::string_view flavour_text) {
		flavour_text_ = flavour_text;
	}
	 ...
	 */
	const std::string& flavourText() const {
		return flavour_text_;
	}
	 ...
	 */
	void addNote(std::string_view note) {
		notes_.emplace(note);
	}
	 ...
	 */
	void setNotes(std::set<std::string> notes) {
	 ...
	 */
	const std::set<std::string>& notes() const {
		return notes_;
	}
	 ...
	 */
	void addRace(const RaceData& race) {
		races_.insert(&race);
	}
	 ...
	 */
	void setRaces(std::set<const RaceData*> races) {
	 ...
	 */
	const std::set<const RaceData*> races() const {
		return races_;
	}
	 ...
	 */
	void addQualifier(std::string_view qualifier, int reduction) {
		qualifiers_.emplace(qualifier, reduction);
	}
	 ...
	 */
	void setQualifiers(std::map<std::string, int> qualifiers) {
	 ...
	 */
	const std::map<std::string, int>& qualifiers() const {
		return qualifiers_;
	}
	 ...
	 */
	void setLifestyle(bool lifestyle) {
	 ...
	 */
	void setTimeToAcquire(int months) {
		time_to_acquire_ = months;
	}
	 ...
	 */
	void setStartingMoneyModifierDice(std::string_view max) {
		starting_money_modifier_dice_ = max;
	}
	 ...
	 */
	const std::string& startingMoneyModifierDice() const {
		return starting_money_modifier_dice_;
	}
	 ...
	 */
	void addSpecial(std::string_view special, int chance) {
		specials_.emplace_back(std::make_pair(special, chance));
	}

	/**
	 * @brief Set a collection of special benefits or maluses that may be gained by the package
	 *
	 * The chance parameter is added to the d100 roll when determining whether the special is gained. For example, if a special has a chance of 20, then it would be gained on a roll of 80 or less on a d100. If the chance was -10, then it
	 * would only be gained on a roll of 90 or less. If a special is gained then the chance value should be halved for each subsequent roll to determine whether the special is gained again.
	 *
	 * @param specials Vector of pairs of special benefit or malus descriptions and associated chances
	 */
	void setSpecials(std::vector<std::pair<std::string, int>> specials) {
	 ...
	 */
	const std::vector<std::pair<std::string, int>>& specials() const {
		return specials_;
	}
	 ...
	 */
	void addStatGain(StatType::Type stat) {
		stat_gains_.emplace(stat);
	}
	 ...
	 */
	void setStatGains(std::set<StatType::Type> stat_gains) {
		stat_gains_ = std::move(stat_gains);
	}
	 ...
	 */
	const std::set<StatType::Type>& statGains() const {
		return stat_gains_;
	}
	 ...
	 */
	void setRealmStatGain(bool realmStatGain) {
		realmStatGain_ = realmStatGain;
	}
	 ...
	 */
	bool realmStatGain() const {
		return realmStatGain_;
	}
	 ...
	 */
	void setStatGainChoices(EnumChoice<StatType::Type> choices) {
		stat_gain_choices_ = choices;
	}
	 ...
	 */
	const EnumChoice<StatType::Type> statGainChoices() const {
		return stat_gain_choices_;
	}
	...
	void addSkillRank(SubcategoriedSkillData& skill, int ranks) {
		if (isRankSkill(skill.skillData(), skill.subcategory()))
			throw InvalidSkillRank("There is already a rank set for skill " + skill.id());
	 ...
	 */
	void setSkillRanks(std::map<const SubcategoriedSkillData*, int> skill_ranks) {
		skill_ranks_ = std::move(skill_ranks);
	}
	 ...
	 */
	const std::map<const SubcategoriedSkillData*, int>& skillRanks() const {
		return skill_ranks_;
	}
	 ...
	 */
	void setSkillRankChoices(std::map<GameRuleDataChoice<SubcategoriedSkillData>, int> skill_rank_choices) {
		skill_rank_choices_ = std::move(skill_rank_choices);
	}
	 ...
	 */
	const std::map<GameRuleDataChoice<SubcategoriedSkillData>, int>& skillRankChoices() const {
		return skill_rank_choices_;
	}
	 ...
	 */
	void setSkillCategoryRanks(std::map<const SkillCategoryData*, int> skill_category_ranks) {
		skill_category_ranks_ = std::move(skill_category_ranks);
	}
	 ...
	 */
	const std::map<const SkillCategoryData*, int>& skillCategoryRanks() const {
		return skill_category_ranks_;
	}
	 ...
	 *
	 * Allows the player to select one or more skills from a single category to receive the skill ranks. This is typically used for general skills where the character can select a category of skills and then one or more specific skills
	 * within that category to gain the ranks in.
	 *
	 * @param skill_category_multi_skill_rank_choices Tuple containing a SkillCategoryData pointer representing the skill category from which the player can choose to gain the skill ranks, an integer representing the number of skill ranks
	 * that can be gained, and an integer representing the number of skills that can be chosen to gain the skill ranks.
	 */
	void setSkillCategoryMultiSkillRankChoices(std::set<CategoryMultiSkillRankChoice> skill_category_multi_skill_rank_choices) {
		skill_category_multi_skill_rank_choices_ = std::move(skill_category_multi_skill_rank_choices);
	}
	 ...
	 * @return Tuple containing a SkillCategoryData pointer representing the skill category from which the player can choose to gain the skill ranks, an integer representing the number of skill ranks that can be gained, and an integer
	 * representing the number of skills that can be chosen to gain the skill ranks.
	 */
	const std::set<CategoryMultiSkillRankChoice>& skillCategoryMultiSkillRankChoices() const {
		return skill_category_multi_skill_rank_choices_;
	}
	 ...
	 *
	 * Allows the player to select one or more skills from a single group to receive the skill ranks. This is typically used for weapon skills where the character can select a category of weapons and then one or more specific weapons within
	 * that category to gain the ranks in.
	 *
	 * @param skill_group_multi_skill_rank_choices Vector containing GroupMultiSkillRankChoice structs, each representing a skill group from which the player can choose to gain skill ranks, along with the number of skill ranks that can be
	 * gained and the number of skills that can be chosen to gain those ranks.
	 */
	void setSkillGroupMultiSkillRankChoices(std::vector<GroupMultiSkillRankChoice> skill_group_multi_skill_rank_choices) {
		skill_group_multi_skill_rank_choices_ = std::move(skill_group_multi_skill_rank_choices);
	}
	 ...
	 * that category to gain the ranks in.
	 *
	 * @return Set containing GroupMultiSkillRankChoice structs, each representing a skill group from which the player can choose to gain skill ranks, along with the number of skill ranks that can be gained and the number of skills that can
	 * be chosen to gain those ranks.
	 */
	const std::vector<GroupMultiSkillRankChoice>& skillGroupMultiSkillRankChoices() const {
		return skill_group_multi_skill_rank_choices_;
	}
	 ...
	 */
	void setSkillGroupCategoryAndSkillRanks(std::vector<SkillGroupCategoryAndSkillRankChoice> skill_group_category_and_skill_ranks) {
		skill_group_category_and_skill_ranks_ = std::move(skill_group_category_and_skill_ranks);
	}
	 ...
	 */
	const std::vector<SkillGroupCategoryAndSkillRankChoice>& skillGroupCategoryAndSkillRanks() const {
		return skill_group_category_and_skill_ranks_;
	}
	 ...
	 */
	void setSpellListChoices(std::set<SpellListChoices> spell_list_choices) {
		spell_list_choices_ = std::move(spell_list_choices);
	}
	 ...
	 */
	void addSpellListChoices(std::set<SpellListChoices> spell_list_choices) {
		spell_list_choices_.insert(spell_list_choices.begin(), spell_list_choices.end());
	}
	 ...
	 */
	const std::set<SpellListChoices>& spellListChoices() const {
		return spell_list_choices_;
	}
	 ...
	 */
	void setSpellListCategoryChoices(std::set<SpellListCategoryChoices> spell_list_category_choices) {
		spell_list_category_choices_ = std::move(spell_list_category_choices);
	}
	 ...
	 */
	const std::set<SpellListCategoryChoices>& spellListCategoryChoices() const {
		return spell_list_category_choices_;
	}
	 ...
	 */
	void setLifestyleSkills(std::set<const SubcategoriedSkillData*> lifestyle_skills) {
		lifestyle_skills_ = std::move(lifestyle_skills);
	}
	 ...
	 */
	const std::set<const SubcategoriedSkillData*>& lifestyleSkills() const {
		return lifestyle_skills_;
	}
	 ...
	 */
	void setLifestyleSkillCategories(std::set<const SkillCategoryData*> lifestyle_skill_categories) {
		lifestyle_skill_categories_ = std::move(lifestyle_skill_categories);
	}
	 ...
	 */
	const std::set<const SkillCategoryData*>& lifestyleSkillCategories() const {
		return lifestyle_skill_categories_;
	}
	 ...
	 */
	void setLifestyleSkillGroups(std::set<const SkillGroupData*> lifestyle_skill_groups) {
		lifestyle_skill_groups_ = std::move(lifestyle_skill_groups);
	}
	 ...
	 */
	const std::set<const SkillGroupData*>& lifestyleSkillGroups() const {
		return lifestyle_skill_groups_;
	}
	 ...
	 */
	void setLifestyleCategorySkillChoices(std::set<GameRuleDataChoice<SkillCategoryData>> lifestyle_category_skill_choices) {
		lifestyle_category_skill_choices_ = std::move(lifestyle_category_skill_choices);
	}
	 ...
	 */
	const std::set<GameRuleDataChoice<SkillCategoryData>>& lifestyleCategorySkillChoices() const {
		return lifestyle_category_skill_choices_;
	}
	 ...
	 */
	void setLanguageChoices(std::map<GameRuleDataChoice<LanguageData>, int> language_choices) {
		language_choices_ = std::move(language_choices);
	}
	 ...
	 */
	const std::map<GameRuleDataChoice<LanguageData>, int>& languageChoices() const {
