...
class LanguageRanks {
public:
	LanguageRanks() = default; /**< Default constructor */
	 ...
	 * @param language LanguageData reference to the language
	 */
	void setLanguage(const rule::LanguageData& language) {
		language_ = &language;
	}

	/**
	 * @brief Get the language the ability is in
	 * @return LanguageData reference to the language
	 */
	const rule::LanguageData& language() const {
		return *language_;
	}
	 ...
	 */
	void updateSomaticRanks(int ranks);

	/**
	 * @brief Get the number of somatic ranks
	 * @return somatic ranks
	 */
	int somaticRanks() const {
		return isSomatic() ? std::max(0, somatic_) : 0;
	}
	 ...
	 * @return spoken ranks
	 */
	int spokenRanks() const {
		return isSpoken() ? std::max(0, spoken_) : 0;
	}
	 ...
	 */
	int writtenRanks() const {
		return isWritten() ? std::max(0, written_) : 0;
	}
