...
namespace Character {
	...
	int getInitialPotentialStatValue(int temp_value) {
		if (temp_value <= 24) return 20 + Dice(10).roll(8).result();
		if (temp_value <= 34) return 30 + Dice(10).roll(7).result();
		if (temp_value <= 44) return 40 + Dice(10).roll(6).result();
		if (temp_value <= 54) return 50 + Dice(10).roll(5).result();
		if (temp_value <= 64) return 60 + Dice(10).roll(4).result();
		if (temp_value <= 74) return std::max(temp_value, 70 + Dice(10).roll(3).result());
		if (temp_value <= 84) return std::max(temp_value, 80 + Dice(10).roll(2).result());
		if (temp_value <= 91) return 90 + Dice(10).roll().result();
		if (temp_value == 92) return 91 + Dice(9).roll().result();
		if (temp_value == 93) return 92 + Dice(8).roll().result();
		if (temp_value == 94) return 93 + Dice(7).roll().result();
		if (temp_value == 95) return 94 + Dice(6).roll().result();
		if (temp_value == 96) return 95 + Dice(5).roll().result();
		if (temp_value == 97) return 96 + Dice(4).roll().result();
		if (temp_value == 98) return 97 + Dice(3).roll().result();
		if (temp_value == 99) return 98 + Dice(2).roll().result();
		return 99 + Dice(2).roll().result();
	};

	int getStatGain(int difference) {
		// This uses two dice rolls and the difference between the stats determines how they are used.
		int roll1 = Dice(10).roll().result();
		int roll2 = Dice(10).roll().result();
